                ...
                <form class="card-body" action="{{  route('update_skill',$skill->id) }}" method="POST" enctype="multipart/form-data">
              @csrf
                <div class="row g-3">
                  <div class="col-md-6">
                    <label class="form-label" for="multicol-username">اسم المهارة</label>
                    <input name="name"   type="text"value="{{ $skill->name }}" id="multicol-username" class="form-control" placeholder="" />
                    @error('name')
               ...
               <label class="form-label" for="multicol-email"> المستوى</label>
                    <select name="level"  value="{{ $skill->level }}" class="form-select item-details mb-2">
